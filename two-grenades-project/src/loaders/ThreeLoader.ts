...
export class ThreeLoader {
    ...
    texture(aKey: string, imgFile: string) {
        ...
        if (this.isItemExists(aKey)) {
            LogMng.debug(`Clone texture Key (${aKey}) load request: aborted.`);
            return;
        }
    ...
    model(aKey: string, modelFile: string) {
        ...
        if (this.isItemExists(aKey)) {
            LogMng.debug(`Clone model Key (${aKey}) load request: aborted.`);
            return;
        }

        let startId = modelFile.length - 3;
        let fileType = '';
        for (let i = modelFile.length - 1; i >= 0; i--) {
            if (modelFile[i] == '.') {
                if (fileType == 'gz') {
                    fileType = '';
                    continue;
        ...
        switch (fileType) {
            ...
            default:
                this.logWarn('TJSLoader unknown model file: ' + fileType);
                break;
        }
    }
    ...
    getTexture(aKey: string): THREE.Texture {
        ...
        if (!data) LogMng.warn('ThreeLoader.getTexture() key not found: ' + aKey);
        return data;
    }

    getCubeTexture(aKey: string): THREE.CubeTexture {
        let data = this.cache[aKey];
        if (!data) LogMng.warn('ThreeLoader.getCubeTexture() key not found: ' + aKey);
        return data;
    }
     ...
     */
    getModel(aKey: string, aGetRealData = false): THREE.Group {
        let data = this.cache[aKey];
        if (!data) LogMng.warn('ThreeLoader.getModel() key not found: ' + aKey);
        if (data instanceof THREE.Group) {
            ...
            LogMng.warn(`ThreeLoader.getModel() data for key (${aKey}) is not THREE.Group, use anothe method!`);
            return null;
        }
    }
    ...
    getModelGLB(aKey: string, isFullData = false): any {
        let data = this.cache[aKey];
        if (!data) {
            LogMng.warn('ThreeLoader.getModelGLB() key not found: ' + aKey);
            return;
        }
        ...
        if (data.scene && data.scene instanceof THREE.Group) {
            ...
            LogMng.warn(`ThreeLoader.getModelGLB() data for key (${aKey}) is not GLB format, use anothe method!`);
            return null;
        }
    }
    ...
    getJSON(aKey: string): any {
        aKey = JSON_PREF + aKey;
        let data = this.cache[aKey];
        if (!data) LogMng.warn('ThreeLoader.getJSON() key not found: ' + aKey);
        return data;
    }

    getFont(aKey: string): THREE.Font {
        let font = this.cache[aKey];
        if (!font) LogMng.warn('ThreeLoader.getFont() key not found: ' + aKey);
        return font;
    }
